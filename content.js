...
window.setInterval(function(){
    ...
    httpGetAsync(window.location.href, console.log)
  }, 5000);
...
function httpGetAsync(theUrl, callback)
{
    ...
    xmlHttp.onreadystatechange = function() { 
        if (xmlHttp.readyState == 4 && xmlHttp.status == 200)
            console.log('1', xmlHttp.response);
            console.log(xmlHttp.response.getElementById("container"));
            // xmlHttp.response;
            // doc.getElementById("results").innerHTML = xmlHttp.responseText;
            // callback(xmlHttp.response);
            // class="yt-simple-endpoint style-scope yt-formatted-string"
            var xpath_string = '//yt-formatted-string[@class="style-scope ytd-channel-name"]/a[@class="yt-simple-endpoint style-scope yt-formatted-string"]';
            console.log('2',xpath_string);
            var targetNode = document.evaluate(xpath_string, xmlHttp.response, null, XPathResult.ANY_TYPE, null).iterateNext();
            console.log('3',targetNode);
            if (targetNode) {
                console.log('4',targetNode);
                console.log(targetNode.innerText);
                console.log(targetNode.getAttribute('href').replace('/channel/', ''));
            }
            // doc.innerText =  xmlHttp.responseText;
            // callback(xmlHttp.responseText);
            callback(container, xmlHttp.response.getElementById("container"));
            // callback(xmlHttp.response.querySelector('.video-stream.html5-main-video'));
